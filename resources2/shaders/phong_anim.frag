...
layout(set = 1, binding = 1, std140) uniform LightTransforms {
    mat4 m[MAX];
} transforms;

layout(set = 1, binding = 2, std140) uniform LightCount {
    int n;
} count;

layout(set = 2, binding = 0) uniform texture2DArray img;
...
vec3 calc_point_light(Light light, mat4 transform, vec3 col) {
    ...
    float f_specular = pow(max(dot(normalize(-pos), reflect_dir), 0.0), 32);
...
void main() {
    ...
    vec3 col = texture(sampler2DArray(img, samp), vec3(tex * mat.scale, mat.i)).rgb;
