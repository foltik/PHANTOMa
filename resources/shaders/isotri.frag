...
void main() {
    ...
    float repeatSize = 4.;
    float x = uv.x - mod(t, repeatSize / 2.);
    float y = uv.y;
    ...
    // Triangle distance
    float d = sTri(uv + vec2(0,.1), .1 + u.r / 3.0);
    ...
    weight = mix(1.0 + (5.0 * u.weight), 1.2, weight);
    // weight = mix(.5, .33, weight);

    // Offset the line by the weight
    lines -= weight - (3.0 * (1.0 - u.thickness));

    color = vec4(u.c * vec3(1.0 - lines), 1.0);
}
