use super::{load_shader, Component};
...
use rendy::{
    ...
    hal::{self, Backend},
    init::winit::event::{Event, WindowEvent},
    memory::Dynamic,
    mesh::PosColor,
    resource::{Buffer, BufferInfo, DescriptorSetLayout, Escape, Handle},
    shader::{ShaderKind, ShaderSetBuilder, SpirvReflection, SpirvShader},
};

lazy_static::lazy_static! {
    static ref SHADER_REFLECTION: SpirvReflection = SHADERS.reflect().unwrap();

    static ref VERT: SpirvShader = load_shader(include_str!("shader.vert"), ShaderKind::Vertex);
    static ref FRAG: SpirvShader = load_shader(include_str!("shader.frag"), ShaderKind::Fragment);

    static ref SHADERS: ShaderSetBuilder = ShaderSetBuilder::default()
        .with_vertex(&*VERT).unwrap()
...
impl<B: Backend, T: 'static + Sized> Component<B, T> for Triangle {
