...
vec3 glitch_blocks() {
    vec3 c = texture(img, tex).rgb;

    float t = floor(u.t * 0.036);
    float r = rand(vec2(t, 0.0));

    const float f_skew = 0.08 * u.amp * 14.0;
    ...
    for (float i = 0.0; i < 20.0 * f_skew; i += 1.0) {
        ...
        if (inside(tex.y, y, fract(y + h)) == 1.0) {
            ...
            c = texture(img, vec2(tex.x + ofs, tex.y)).rgb;
        }
    }
    ...
    if (r <= 0.33) {
        c.r = texture(img, tex + cofs).r;
    } else if (r <= 0.66) {
        c.g = texture(img, tex + cofs).g;
    } else {
        c.b = texture(img, tex + cofs).b;
    }
...
vec3 glitch_vhs() {
    ...
    vec3 c = texture(img, vec2(x, tex.y)).rgb;
    ...
    c.g = mix(c.r, texture(img, vec2(x + n * 0.05, tex.y)).g, 0.25);
    c.b = mix(c.r, texture(img, vec2(x - n * 0.05, tex.y)).b, 0.25);
